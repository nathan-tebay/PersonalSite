...
function getSavedTheme () {
  const theme = window.localStorage.getItem('tebay_theme') || window.localStorage.getItem('theme')
  if (theme && isValidHex(theme)) return theme
  return THEMES[0].hex
}

function saveTheme (hex) {
  window.localStorage.setItem('tebay_theme', hex)
...
;(function () {
  ...
  const preview = modal.querySelector('.image-modal__image')
  ...
  function getImageLabel (img) {
    const card = img.closest('.render-card')
    const label = card ? card.querySelector('.render-label') : null
  ...
  document.addEventListener('click', function (e) {
    const img = e.target.closest('.render-card img')
    if (!img) return
    openModal(img)
  })
  ...
  document.addEventListener('keydown', function (e) {
    ...
    const img = e.target.closest && e.target.closest('.render-card img')
    if (!img) return
    e.preventDefault()
    openModal(img)
  })
...
;(function () {
  ...
  const iframe = modal.querySelector('.demo-modal__iframe')
  const caption = modal.querySelector('.demo-modal__caption')
  const closeButton = modal.querySelector('.demo-modal__close')
  let lastFocused = null
  ...
  document.addEventListener('click', function (e) {
    const trigger = e.target.closest('[data-demo-src]')
    if (!trigger) return
...
;(function () {
  ...
  const caption = modal.querySelector('.video-modal__caption')
